...
var VSHADER_SOURCE =
    ...
    'uniform vec4 u_Translation;' +    //平移变量
    'void main(){' +
    'gl_Position = a_Position + u_Translation;' +       //attribute变量赋值给gl_Position
    '}';
...
function initVertexBuffer(gl, selectedShape) {
    ...
    if(!u_Translation){
        console.log("获取u_Translation存储位置失败")
        return false;
    }
    /*
        给u_Translation传递数值
        齐次坐标的最后一个分量为1.0，则vec4可以表示为一个三维坐标
        所以下面参数的最后一个分量只能为0.0
     */
